...
<head>
    ...
    <style>
        ...
        .btn-acoes { padding: 8px; cursor: pointer; }
...
<div class="box">
    ...
    @forelse($alunos as $aluno)
        <div class="aluno">
            <form action="{{ route('alunos.update', $aluno) }}" method="POST">
                ...
                <div class="row">
                    <div style="flex:1;">
                        <input type="text" name="nome" disabled value="{{ $aluno->nome }}">
                        <input type="email" name="email" disabled value="{{ $aluno->email }}">
                        <input type="date" name="data_nascimento" disabled value="{{ $aluno->data_nascimento }}">
                    </div>
                </div>
            </form>

            <div class="btn-container">
                <form action="{{ route('alunos.edit', $aluno) }}" method="GET">
                    <button type="submit" class="btn-acoes">Editar</button>
                </form>
    ...
    {{--Criar Novo Aluno--}}
    <div class="btn-criar">
        <a href="{{ route('alunos.create', $aluno) }}">
